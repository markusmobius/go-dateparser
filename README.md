...
```sh
go get github.com/markusmobius/go-dateparser@v1.4.5
```
...
Both APIs default to **MDY**, matching Python dateparser 1.4.3; this corrects
the previous Go DMY default. Set `Configuration.DateOrder` to override it.
Missing components come from `CurrentTime` converted to the requested calendar
using its local calendar date. Only an omitted day is clamped to the month's
last day; an explicit overflowing day retains Python's rollover behavior.

Native conversion tables reproduce Python's `convertdate` Jalali calculations
and `hijridate` Umm al-Qura month boundaries. Umm al-Qura supports years
1343-1500 AH, with reference dates from 1924-08-01 through 2077-11-16 CE.
Out-of-range reference dates return an error even when the input supplies all
components, as in Python. The exact generated bounds and provenance are in
[internal/parser/calendars/data.json](internal/parser/calendars/data.json).
Neither Python nor its calendar packages are runtime dependencies.
