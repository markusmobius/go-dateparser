// Code is generated by script; DO NOT EDIT.

package data

import "regexp"

var lo_Locale = merge(nil, LocaleData{
	Name:      "lo",
	DateOrder: "DMY",
	SkipWords: []string{"'", ",", "-", ".", "/", ";", "@", "[", "]", "|"},
	Translations: map[string]string{
		"ເດອນແລວ": "1 month ago",
		"ອາທດແລວ": "1 week ago",
		"ເດອນຫນາ": "in 1 month",
		"ອາທດຫນາ": "in 1 week",
		"ວນອງຄານ": "tuesday",
		"ຊວໂມງນ":  "0 hour ago",
		"ກອນທຽງ":  "am",
		"ພດສະພາ":  "may",
		"ວນອາທດ":  "sunday",
		"ວນພະຫດ":  "thursday",
		"ເດອນນ":   "0 month ago",
		"ອາທດນ":   "0 week ago",
		"ຊວໂມງ":   "hour",
		"ມງກອນ":   "january",
		"ກລະກດ":   "july",
		"ຫງທຽງ":   "pm",
		"ວນເສາ":   "saturday",
		"ອງຄານ":   "tuesday",
		"ນາທນ":    "0 minute ago",
		"ຕອນນ":    "0 second ago",
		"ມວານ":    "1 day ago",
		"ປກາຍ":    "1 year ago",
		"ເມສາ":    "april",
		"ສງຫາ":    "august",
		"ທນວາ":    "december",
		"ກມພາ":    "february",
		"ວນສກ":    "friday",
		"ປຫນາ":    "in 1 year",
		"ມຖນາ":    "june",
		"ວນຈນ":    "monday",
		"ເດອນ":    "month",
		"ພະຈກ":    "november",
		"ວນາທ":    "second",
		"ກນຍາ":    "september",
		"ພະຫດ":    "thursday",
		"ວນພດ":    "wednesday",
		"ອາທດ":    "week",
		"gmt":     "gmt",
		"ມອນ":     "in 1 day",
		"ມນາ":     "march",
		"ນາທ":     "minute",
		"ຕລາ":     "october",
		"ເສາ":     "saturday",
		"utc":     "utc",
		"ມນ":      "0 day ago",
		"ປນ":      "0 year ago",
		"am":      "am",
		"ມສ":      "april",
		"ສຫ":      "august",
		"ທວ":      "december",
		"ກພ":      "february",
		"ສກ":      "friday",
		"ຊມ":      "hour",
		"ມກ":      "january",
		"ກລ":      "july",
		"ມຖ":      "june",
		"ພພ":      "may",
		"ນທ":      "minute",
		"ຈນ":      "monday",
		"ພຈ":      "november",
		"ຕລ":      "october",
		"pm":      "pm",
		"ກຍ":      "september",
		"ພດ":      "wednesday",
		"'":       "",
		",":       "",
		";":       "",
		"@":       "",
		"[":       "",
		"]":       "",
		"|":       "",
		" ":       " ",
		"+":       "+",
		"-":       "-",
		".":       ".",
		"/":       "/",
		":":       ":",
		"ມ":       "day",
		"ດ":       "month",
		"ວ":       "second",
		"ອ":       "week",
		"ປ":       "year",
		"z":       "z",
	},
	TranslationRegexes: []ReplacementData{
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ຊວໂມງ`), "in $1 hour"},
		{regexp.MustCompile(`(?i)(\d+) ໃນອກ 0 ນາທ`), "in $1 minute"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ເດອນ`), "in $1 month"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ວນາທ`), "in $1 second"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ອາທດ`), "in $1 week"},
		{regexp.MustCompile(`(?i)(\d+) ຊວໂມງກອນ`), "$1 hour ago"},
		{regexp.MustCompile(`(?i)(\d+) ເດອນກອນ`), "$1 month ago"},
		{regexp.MustCompile(`(?i)(\d+) ວນາທກອນ`), "$1 second ago"},
		{regexp.MustCompile(`(?i)(\d+) ອາທດກອນ`), "$1 week ago"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ຊມ`), "in $1 hour"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ອທ`), "in $1 week"},
		{regexp.MustCompile(`(?i)(\d+) ຊມ ກອນ`), "$1 hour ago"},
		{regexp.MustCompile(`(?i)(\d+) ນທ ກອນ`), "$1 minute ago"},
		{regexp.MustCompile(`(?i)(\d+) ນາທກອນ`), "$1 minute ago"},
		{regexp.MustCompile(`(?i)(\d+) ອທ ກອນ`), "$1 week ago"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ມ`), "in $1 day"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ດ`), "in $1 month"},
		{regexp.MustCompile(`(?i)ໃນອກ (\d+) ປ`), "in $1 year"},
		{regexp.MustCompile(`(?i)(\d+) ດ ກອນ`), "$1 month ago"},
		{regexp.MustCompile(`(?i)(\d+) ວ ກອນ`), "$1 second ago"},
		{regexp.MustCompile(`(?i)ໃນ (\d+) ນທ`), "in $1 minute"},
		{regexp.MustCompile(`(?i)(\d+) ມກອນ`), "$1 day ago"},
		{regexp.MustCompile(`(?i)(\d+) ປກອນ`), "$1 year ago"},
		{regexp.MustCompile(`(?i)ໃນ (\d+) ວ`), "in $1 second"},
	},
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(\d+ ໃນອກ 0 ນາທ|ໃນອກ \d+ ຊວໂມງ|ໃນອກ \d+ ວນາທ|ໃນອກ \d+ ອາທດ|ໃນອກ \d+ ເດອນ|\d+ ຊວໂມງກອນ|\d+ ວນາທກອນ|\d+ ອາທດກອນ|\d+ ເດອນກອນ|ໃນອກ \d+ ຊມ|ໃນອກ \d+ ອທ|\d+ ຊມ ກອນ|\d+ ນທ ກອນ|\d+ ນາທກອນ|\d+ ອທ ກອນ|ໃນອກ \d+ ດ|ໃນອກ \d+ ປ|ໃນອກ \d+ ມ|\d+ ດ ກອນ|\d+ ວ ກອນ|ໃນ \d+ ນທ|\d+ ປກອນ|\d+ ມກອນ|ໃນ \d+ ວ)(\z|\W|_)`),
	RxExactCombined: regexp.MustCompile(`(?i)^(\d+ ໃນອກ 0 ນາທ|ໃນອກ \d+ ຊວໂມງ|ໃນອກ \d+ ວນາທ|ໃນອກ \d+ ອາທດ|ໃນອກ \d+ ເດອນ|\d+ ຊວໂມງກອນ|\d+ ວນາທກອນ|\d+ ອາທດກອນ|\d+ ເດອນກອນ|ໃນອກ \d+ ຊມ|ໃນອກ \d+ ອທ|\d+ ຊມ ກອນ|\d+ ນທ ກອນ|\d+ ນາທກອນ|\d+ ອທ ກອນ|ໃນອກ \d+ ດ|ໃນອກ \d+ ປ|ໃນອກ \d+ ມ|\d+ ດ ກອນ|\d+ ວ ກອນ|ໃນ \d+ ນທ|\d+ ປກອນ|\d+ ມກອນ|ໃນ \d+ ວ)$`),
	RxKnownWords:    regexp.MustCompile(`(?i)^(.*?(?:\A|\W|_|\d))(ວນອງຄານ|ອາທດຫນາ|ອາທດແລວ|ເດອນຫນາ|ເດອນແລວ|ກອນທຽງ|ຊວໂມງນ|ພດສະພາ|ວນພະຫດ|ວນອາທດ|ກລະກດ|ຊວໂມງ|ມງກອນ|ວນເສາ|ຫງທຽງ|ອງຄານ|ອາທດນ|ເດອນນ|ກນຍາ|ກມພາ|ຕອນນ|ທນວາ|ນາທນ|ປກາຍ|ປຫນາ|ພະຈກ|ພະຫດ|ມຖນາ|ມວານ|ວນຈນ|ວນພດ|ວນສກ|ວນາທ|ສງຫາ|ອາທດ|ເດອນ|ເມສາ|gmt|utc|ຕລາ|ນາທ|ມນາ|ມອນ|ເສາ|\+|\.|\[|\]|\||am|pm|ກຍ|ກພ|ກລ|ຈນ|ຊມ|ຕລ|ທວ|ນທ|ປນ|ພຈ|ພດ|ພພ|ມກ|ມຖ|ມນ|ມສ|ສກ|ສຫ| |'|,|-|/|:|;|@|z|ດ|ປ|ມ|ວ|ອ)((?:\z|\W|_|\d).*)$`),
})
