...
var fur_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(\d+ setemanis indaur|\d+ setemane indaur|\d+ zornadis indaur|ca di \d+ setemanis|\d+ seconts indaur|\d+ zornade indaur|ca di \d+ setemane|ca di \d+ zornadis|\d+ minuts indaur|\d+ secont indaur|ca di \d+ seconts|ca di \d+ zornade|\d+ minut indaur|ca di \d+ minuts|ca di \d+ secont|\d+ agns indaur|\d+ oris indaur|ca di \d+ minut|\d+ mes indaur|\d+ ore indaur|ca di \d+ agns|ca di \d+ oris|\d+ an indaur|ca di \d+ mes|ca di \d+ ore|ca di \d+ an)(\z|\W|_)`),
	...
	RxKnownWords:    regexp.MustCompile(`(?i)^(.*?(?:\A|\W|_|\d))(this minute|last month|next month|this month|last week|last year|next week|next year|this hour|this week|this year|dicembar|novembar|setemane|setembar|domenie|martars|miercus|fevrar|otubar|sabide|secont|vinars|avost|avril|doman|joibe|lunis|minut|zenar|jugn|marc|avo|avr|dic|dom|fev|gmt|joi|jug|lui|lun|mai|mar|mes|mie|nov|now|ore|otu|sab|set|utc|vin|vue|zen|\+|\.|\[|\]|\||am|an|di|ir|pm| |'|,|-|/|:|;|@|a|p|z)((?:\z|\W|_|\d).*)$`),
})
