...
var dsb_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(psed \d+ mjasecami|psed \d+ sekundami|psed \d+ tyzenjami|psed \d+ gozinami|psed \d+ minutami|psed \d+ mjasecom|psed \d+ tyzenjom|psed \d+ sekundu|psed \d+ dnjami|psed \d+ gozinu|psed \d+ letami|psed \d+ minutu|za \d+ mjasecow|za \d+ sekundow|za \d+ tyzenjow|psed \d+ dnjom|psed \d+ letom|za \d+ minutow|za \d+ sekundu|psed \d+ mjas|za \d+ gozinu|za \d+ minutu|za \d+ mjasec|psed \d+ dnj|psed \d+ goz|psed \d+ min|psed \d+ sek|psed \d+ tyz|za \d+ dnjow|za \d+ gozin|za \d+ tyzen|za \d+ leto|za \d+ mjas|psed \d+ d|psed \d+ g|psed \d+ l|psed \d+ m|psed \d+ s|za \d+ dnj|za \d+ goz|za \d+ let|za \d+ min|za \d+ sek|za \d+ tyz|za \d+ zen|za \d+ g|za \d+ l|za \d+ m|za \d+ s|za \d+ z)(\z|\W|_)`),
	...
	RxKnownWords:    regexp.MustCompile(`(?i)^(.*?(?:\A|\W|_|\d))(psiducy mjasec|psiducy tyzen|sledny mjasec|sledny tyzen|this minute|ten mjasec|wotpołdnja|dopołdnja|ponjezele|september|septembra|ten tyzen|this hour|december|decembra|februara|nowember|nowembra|awgusta|februar|januara|njezela|oktober|oktobra|sekunda|stwortk|wałtora|apryla|awgust|gozina|januar|julija|junija|letosa|minuta|mjasec|sobota|srjoda|apryl|julij|junij|merca|tyzen|witse|zinsa|znowa|cora|leto|maja|merc|mjas|petk|łoni|apr|awg|dec|feb|gmt|goz|jan|jul|jun|maj|mer|min|nje|now|okt|pet|pon|sek|sep|sob|srj|stw|tyz|utc|wał|zen|\+|\.|\[|\]|\||am|pm| |'|,|-|/|:|;|@|g|l|m|s|z)((?:\z|\W|_|\d).*)$`),
})
