// Code is generated by script; DO NOT EDIT.

package data

import "regexp"

var bo_Locale = LocaleData{
	Name:      "bo",
	DateOrder: "YMD",
	SkipWords: []string{"'", ",", "-", ".", "/", ";", "@", "[", "]", "|"},
	Translations: map[*regexp.Regexp]string{
		regexp.MustCompile(`(\A|\W|_)དེ་རིང་(\z|\W|_)`):          "${1}0 day ago${2}",
		regexp.MustCompile(`(\A|\W|_)this hour(\z|\W|_)`):        "${1}0 hour ago${2}",
		regexp.MustCompile(`(\A|\W|_)this minute(\z|\W|_)`):      "${1}0 minute ago${2}",
		regexp.MustCompile(`(\A|\W|_)this month(\z|\W|_)`):       "${1}0 month ago${2}",
		regexp.MustCompile(`(\A|\W|_)now(\z|\W|_)`):              "${1}0 second ago${2}",
		regexp.MustCompile(`(\A|\W|_)this week(\z|\W|_)`):        "${1}0 week ago${2}",
		regexp.MustCompile(`(\A|\W|_)this year(\z|\W|_)`):        "${1}0 year ago${2}",
		regexp.MustCompile(`(\A|\W|_)ཁས་ས་(\z|\W|_)`):            "${1}1 day ago${2}",
		regexp.MustCompile(`(\A|\W|_)last month(\z|\W|_)`):       "${1}1 month ago${2}",
		regexp.MustCompile(`(\A|\W|_)last week(\z|\W|_)`):        "${1}1 week ago${2}",
		regexp.MustCompile(`(\A|\W|_)last year(\z|\W|_)`):        "${1}1 year ago${2}",
		regexp.MustCompile(`(\A|\W|_)སྔ་དྲོ་(\z|\W|_)`):          "${1}am${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༤(\z|\W|_)`):             "${1}april${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཞི་པ(\z|\W|_)`):       "${1}april${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཞི་པ་(\z|\W|_)`):      "${1}april${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༨(\z|\W|_)`):             "${1}august${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བརྒྱད་པ(\z|\W|_)`):     "${1}august${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བརྒྱད་པ་(\z|\W|_)`):    "${1}august${2}",
		regexp.MustCompile(`(\A|\W|_)ཉིན།(\z|\W|_)`):             "${1}day${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༡༢(\z|\W|_)`):            "${1}december${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཅུ་གཉིས་པ(\z|\W|_)`):  "${1}december${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཅུ་གཉིས་པ་(\z|\W|_)`): "${1}december${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༢(\z|\W|_)`):             "${1}february${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་གཉིས་པ(\z|\W|_)`):      "${1}february${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་གཉིས་པ་(\z|\W|_)`):     "${1}february${2}",
		regexp.MustCompile(`(\A|\W|_)གཟའ་པ་སངས་(\z|\W|_)`):       "${1}friday${2}",
		regexp.MustCompile(`(\A|\W|_)པ་སངས་(\z|\W|_)`):           "${1}friday${2}",
		regexp.MustCompile(`(\A|\W|_)ཆུ་ཚོད་(\z|\W|_)`):          "${1}hour${2}",
		regexp.MustCompile(`(\A|\W|_)སང་ཉིན་(\z|\W|_)`):          "${1}in 1 day${2}",
		regexp.MustCompile(`(\A|\W|_)next month(\z|\W|_)`):       "${1}in 1 month${2}",
		regexp.MustCompile(`(\A|\W|_)next week(\z|\W|_)`):        "${1}in 1 week${2}",
		regexp.MustCompile(`(\A|\W|_)next year(\z|\W|_)`):        "${1}in 1 year${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༡(\z|\W|_)`):             "${1}january${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་དང་པོ(\z|\W|_)`):       "${1}january${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་དང་པོ་(\z|\W|_)`):      "${1}january${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༧(\z|\W|_)`):             "${1}july${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བདུན་པ(\z|\W|_)`):      "${1}july${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བདུན་པ་(\z|\W|_)`):     "${1}july${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༦(\z|\W|_)`):             "${1}june${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་དྲུག་པ(\z|\W|_)`):      "${1}june${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་དྲུག་པ་(\z|\W|_)`):     "${1}june${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༣(\z|\W|_)`):             "${1}march${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་གསུམ་པ(\z|\W|_)`):      "${1}march${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་གསུམ་པ་(\z|\W|_)`):     "${1}march${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༥(\z|\W|_)`):             "${1}may${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་ལྔ་པ(\z|\W|_)`):        "${1}may${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་ལྔ་པ་(\z|\W|_)`):       "${1}may${2}",
		regexp.MustCompile(`(\A|\W|_)གཟའ་ཟླ་བ་(\z|\W|_)`):        "${1}monday${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་(\z|\W|_)`):            "${1}monday${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༡༡(\z|\W|_)`):            "${1}november${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཅུ་གཅིག་པ(\z|\W|_)`):  "${1}november${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཅུ་གཅིག་པ་(\z|\W|_)`): "${1}november${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༡༠(\z|\W|_)`):            "${1}october${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཅུ་པ(\z|\W|_)`):       "${1}october${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་བཅུ་པ་(\z|\W|_)`):      "${1}october${2}",
		regexp.MustCompile(`(\A|\W|_)ཕྱི་དྲོ་(\z|\W|_)`):         "${1}pm${2}",
		regexp.MustCompile(`(\A|\W|_)གཟའ་སྤེན་པ་(\z|\W|_)`):      "${1}saturday${2}",
		regexp.MustCompile(`(\A|\W|_)སྤེན་པ་(\z|\W|_)`):          "${1}saturday${2}",
		regexp.MustCompile(`(\A|\W|_)སྐར་ཆ།(\z|\W|_)`):           "${1}second${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་༩(\z|\W|_)`):             "${1}september${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་དགུ་པ(\z|\W|_)`):       "${1}september${2}",
		regexp.MustCompile(`(\A|\W|_)ཟླ་བ་དགུ་པ་(\z|\W|_)`):      "${1}september${2}",
		regexp.MustCompile(`(\A|\W|_)གཟའ་ཉི་མ་(\z|\W|_)`):        "${1}sunday${2}",
		regexp.MustCompile(`(\A|\W|_)ཉི་མ་(\z|\W|_)`):            "${1}sunday${2}",
		regexp.MustCompile(`(\A|\W|_)གཟའ་ཕུར་བུ་(\z|\W|_)`):      "${1}thursday${2}",
		regexp.MustCompile(`(\A|\W|_)ཕུར་བུ་(\z|\W|_)`):          "${1}thursday${2}",
		regexp.MustCompile(`(\A|\W|_)གཟའ་མིག་དམར་(\z|\W|_)`):     "${1}tuesday${2}",
		regexp.MustCompile(`(\A|\W|_)མིག་དམར་(\z|\W|_)`):         "${1}tuesday${2}",
		regexp.MustCompile(`(\A|\W|_)གཟའ་ལྷག་པ་(\z|\W|_)`):       "${1}wednesday${2}",
		regexp.MustCompile(`(\A|\W|_)ལྷག་པ་(\z|\W|_)`):           "${1}wednesday${2}",
		regexp.MustCompile(`(\A|\W|_)week(\z|\W|_)`):             "${1}week${2}",
		regexp.MustCompile(`(\A|\W|_)ལོ།(\z|\W|_)`):              "${1}year${2}",
	},
}

var bo_IN_Locale = LocaleData{
	Name:      "bo-IN",
	Parent:    &bo_Locale,
	DateOrder: "YMD",
}
