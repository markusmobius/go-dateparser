...
var bs_Locale = merge(nil, LocaleData{
	Name:      "bs",
	DateOrder: "DMY.",
	SkipWords: []string{"'", ",", "-", ".", "/", ";", "@", "[", "]", "|"},
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(prije \d+ mjeseci|prije \d+ sedmica|prije \d+ sedmicu|prije \d+ sekundi|prije \d+ sekundu|prije \d+ godina|prije \d+ godinu|prije \d+ minuta|prije \d+ minutu|prije \d+ mjesec|prije \d+ dana|prije \d+ sati|za \d+ mjeseci|za \d+ sedmica|za \d+ sedmicu|za \d+ sekundi|za \d+ sekundu|prije \d+ dan|prije \d+ god|prije \d+ min|prije \d+ sat|prije \d+ sed|prije \d+ sek|za \d+ godina|za \d+ godinu|za \d+ minuta|za \d+ minutu|za \d+ mjesec|prije \d+ mj|prije \d+ d|prije \d+ g|za \d+ dana|za \d+ sati|za \d+ dan|za \d+ god|za \d+ min|za \d+ sat|za \d+ sed|za \d+ sek|za \d+ mj|za \d+ d|za \d+ g)(\z|\W|_)`),
