...
var sk_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|[^\pL\pM\d]|_)(pred \d+ sekundami|pred \d+ hodinami|pred \d+ mesiacmi|pred \d+ mesiacom|pred \d+ minutami|pred \d+ sekundou|pred \d+ tyzdnami|pred \d+ hodinou|pred \d+ minutou|pred \d+ tyzdnom|o \d+ mesiacov|pred \d+ dnami|pred \d+ rokmi|pred \d+ rokom|o \d+ sekundu|o \d+ tyzdnov|pred \d+ dnom|o \d+ hodinu|o \d+ mesiac|o \d+ minutu|o \d+ sekund|o \d+ tyzden|pred \d+ mes|pred \d+ min|pred \d+ tyz|o \d+ hodin|o \d+ minut|o \d+ rokov|pred \d+ d|pred \d+ h|pred \d+ r|pred \d+ s|o \d+ den|o \d+ dni|o \d+ mes|o \d+ min|o \d+ rok|o \d+ tyz|o \d+ d|o \d+ h|o \d+ r|o \d+ s)(\z|[^\pL\pM\d]|_)`),
	...
	RxKnownWords:    regexp.MustCompile(`(?i)^(.*?(?:\A|[^\pL\pM\d]|_|\d))(v tejto hodine|v tejto minute|buduci mesiac|buduci tyzden|minuly mesiac|minuly tyzden|tento mesiac|tento tyzden|buduci rok|minuly rok|september|septembra|tento rok|december|decembra|februara|november|novembra|pondelok|augusta|februar|januara|oktober|oktobra|sekunda|stvrtok|aprila|august|hodina|januar|mesiac|minuta|nedela|piatok|sobota|streda|tyzden|utorok|zajtra|april|marca|marec|teraz|vcera|dnes|jula|juna|maja|apr|aug|dec|den|feb|gmt|jan|jul|jun|maj|mar|mes|min|nov|okt|rok|sep|tyz|utc|\+|\.|\[|\]|\||am|ne|pi|pm|po|so|st|ut| |'|,|-|/|:|;|@|d|h|r|s|z)((?:\z|[^\pL\pM\d]|_|\d).*)$`),
})
