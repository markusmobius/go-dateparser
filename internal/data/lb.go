...
var lb_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(virun \d+ minutt|virun \d+ sekonn|virun \d+ mount|virun \d+ stonn|virun \d+ joer|virun \d+ woch|an \d+ minutt|an \d+ sekonn|virun \d+ dag|virun \d+ min|virun \d+ sek|an \d+ mount|an \d+ stonn|virun \d+ st|an \d+ joer|an \d+ woch|virun \d+ d|virun \d+ j|virun \d+ m|virun \d+ w|an \d+ dag|an \d+ min|an \d+ sek|an \d+ st|an \d+ d|an \d+ j|an \d+ m|an \d+ w)(\z|\W|_)`),
