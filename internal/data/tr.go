...
var tr_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(\d+ dakika sonra|\d+ saniye sonra|\d+ dakika once|\d+ hafta sonra|\d+ saniye once|\d+ hafta once|\d+ saat sonra|\d+ gun sonra|\d+ saat once|\d+ yıl sonra|\d+ ay sonra|\d+ dk sonra|\d+ gun once|\d+ hf sonra|\d+ sa sonra|\d+ sn sonra|\d+ yıl once|\d+ ay once|\d+ dk once|\d+ hf once|\d+ sa once|\d+ sn once)(\z|\W|_)`),
