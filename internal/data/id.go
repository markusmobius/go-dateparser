...
var id_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(\d+ minggu yang lalu|\d+ bulan yang lalu|\d+ detik yang lalu|\d+ menit yang lalu|\d+ tahun yang lalu|\d+ hari yang lalu|\d+ jam yang lalu|dalam \d+ minggu|dalam \d+ bulan|dalam \d+ detik|dalam \d+ menit|dalam \d+ tahun|dalam \d+ hari|dalam \d+ jam|\d+ bln lalu|\d+ dtk lalu|\d+ jam lalu|\d+ mgg lalu|\d+ mnt lalu|\d+ thn lalu|dalam \d+ h|dlm \d+ bln|dlm \d+ dtk|dlm \d+ mgg|dlm \d+ mnt|dlm \d+ thn|\d+ h lalu)(\z|\W|_)`),
	...
	RxKnownWords:    regexp.MustCompile(`(?i)^(.*?(?:\A|\W|_|\d))(bulan berikutnya|kemarin lusa|minggu depan|minggu lalu|tahun depan|bulan lalu|minggu ini|tahun lalu|baru saja|bulan ini|menit ini|september|tahun ini|desember|februari|hari ini|november|sekarang|seminggu|agustus|jam ini|januari|kemarin|oktober|sebulan|setahun|tanggal|minggu|sehari|selasa|april|besok|bulan|dalam|detik|jumat|kamis|maret|menit|pukul|sabtu|senin|tahun|ahad|hari|juli|juni|lalu|rabu|sept|yang|agt|agu|apr|bln|des|dtk|feb|gmt|jam|jan|jul|jum|jun|kam|mar|mei|mgg|min|mnt|nov|okt|rab|sab|sel|sen|sep|thn|utc|\+|\.|\[|\]|\||am|pm| |'|,|-|/|:|;|@|d|h|j|m|z)((?:\z|\W|_|\d).*)$`),
})
