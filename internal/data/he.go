// Code is generated by script; DO NOT EDIT.

package data

import "regexp"

var he_Locale = merge(nil, LocaleData{
	Name:      "he",
	DateOrder: "DMY",
	SkipWords: []string{"'", ",", "-", ".", "/", ";", "@", "[", "]", "|", "ב-", "בסביבות", "בערך", "בקירוב", "בשעה", "ה-"},
	Simplifications: []ReplacementData{
		{regexp.MustCompile(`(?i)(\A|\W|_)אחרי ה?צהריי?ם(\z|\W|_)`), "${1}pm${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)לפנות בוקר(\z|\W|_)`), "${1}am${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)לפנות ערב(\z|\W|_)`), "${1}pm${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)שבועיי?ם(\z|\W|_)`), "${1}2 שבועות${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)אחר חצות(\z|\W|_)`), "${1}am${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)בצהריי?ם(\z|\W|_)`), "${1}pm${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)חודשיים(\z|\W|_)`), "${1}2 חודשים${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)יומיי?ם(\z|\W|_)`), "${1}2 ימים${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)שנתיי?ם(\z|\W|_)`), "${1}2 שנים${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)שעתיי?ם(\z|\W|_)`), "${1}2 שעות${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)ו(\w+)(\z|\W|_)`), "${1}${2}${3}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)וחודש(\z|\W|_)`), "${1}1 חודש${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)ושבוע(\z|\W|_)`), "${1}1 שבוע${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)שלשום(\z|\W|_)`), "${1}2 ימים${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)בבוקר(\z|\W|_)`), "${1}am${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)אחה"צ(\z|\W|_)`), "${1}pm${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)ויום(\z|\W|_)`), "${1}1 יום${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)ושנה(\z|\W|_)`), "${1}1 שנה${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)חצות(\z|\W|_)`), "${1}12 am${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)בערב(\z|\W|_)`), "${1}pm${2}"},
		{regexp.MustCompile(`(?i)(\A|\W|_)מחר(\z|\W|_)`), "${1}בעוד 1 יום${2}"},
	},
	Translations: map[string]string{
		"החודש שעבר": "1 month ago",
		"השבוע שעבר": "1 week ago",
		"השנה שעברה": "1 year ago",
		"החודש הבא":  "in 1 month",
		"השבוע הבא":  "in 1 week",
		"השנה הבאה":  "in 1 year",
		"יום ראשון":  "sunday",
		"יום חמישי":  "thursday",
		"יום שלישי":  "tuesday",
		"יום רביעי":  "wednesday",
		"יום שישי":   "friday",
		"באוקטובר":   "october",
		"לאוקטובר":   "october",
		"בסביבות":    "",
		"בשעה זו":    "0 hour ago",
		"בדקה זו":    "0 minute ago",
		"באוגוסט":    "august",
		"לאוגוסט":    "august",
		"בפברואר":    "february",
		"לפברואר":    "february",
		"יום שני":    "monday",
		"בנובמבר":    "november",
		"לנובמבר":    "november",
		"אוקטובר":    "october",
		"יום שבת":    "saturday",
		"בספטמבר":    "september",
		"לספטמבר":    "september",
		"בקירוב":     "",
		"לפנה״צ":     "am",
		"באפריל":     "april",
		"לאפריל":     "april",
		"אוגוסט":     "august",
		"בדצמבר":     "december",
		"לדצמבר":     "december",
		"פברואר":     "february",
		"יום ו׳":     "friday",
		"בינואר":     "january",
		"לינואר":     "january",
		"יום ב׳":     "monday",
		"חודשים":     "month",
		"נובמבר":     "november",
		"ספטמבר":     "september",
		"יום א׳":     "sunday",
		"יום ה׳":     "thursday",
		"יום ג׳":     "tuesday",
		"יום ד׳":     "wednesday",
		"שבועות":     "week",
		"החודש":      "0 month ago",
		"עכשיו":      "0 second ago",
		"השבוע":      "0 week ago",
		"אתמול":      "1 day ago",
		"אפריל":      "april",
		"דצמבר":      "december",
		"יום ו":      "friday",
		"ינואר":      "january",
		"ביולי":      "july",
		"ליולי":      "july",
		"ביוני":      "june",
		"ליוני":      "june",
		"במארס":      "march",
		"למארס":      "march",
		"יום ב":      "monday",
		"בחודש":      "month",
		"אחה״צ":      "pm",
		"שניות":      "second",
		"שנייה":      "second",
		"יום א":      "sunday",
		"ראשון":      "sunday",
		"חמישי":      "thursday",
		"יום ה":      "thursday",
		"יום ג":      "tuesday",
		"שלישי":      "tuesday",
		"יום ד":      "wednesday",
		"רביעי":      "wednesday",
		"בערך":       "",
		"בשעה":       "",
		"היום":       "0 day ago",
		"השנה":       "0 year ago",
		"לפני":       "ago",
		"אפר׳":       "april",
		"אוג׳":       "august",
		"ביום":       "day",
		"ימים":       "day",
		"דצמ׳":       "december",
		"פבר׳":       "february",
		"שישי":       "friday",
		"שעות":       "hour",
		"בעוד":       "in",
		"ינו׳":       "january",
		"יולי":       "july",
		"יוני":       "june",
		"במרס":       "march",
		"במרץ":       "march",
		"למרס":       "march",
		"למרץ":       "march",
		"מארס":       "march",
		"במאי":       "may",
		"למאי":       "may",
		"דקות":       "minute",
		"חודש":       "month",
		"נוב׳":       "november",
		"אוק׳":       "october",
		"שניה":       "second",
		"ספט׳":       "september",
		"שבוע":       "week",
		"בשנה":       "year",
		"שנים":       "year",
		"אפר":        "april",
		"אוג":        "august",
		"יום":        "day",
		"דצמ":        "december",
		"פבר":        "february",
		"gmt":        "gmt",
		"שעה":        "hour",
		"שע׳":        "hour",
		"עוד":        "in",
		"מחר":        "in 1 day",
		"ינו":        "january",
		"יול":        "july",
		"יונ":        "june",
		"מרס":        "march",
		"מרץ":        "march",
		"מאי":        "may",
		"דקה":        "minute",
		"דק׳":        "minute",
		"שני":        "monday",
		"חו׳":        "month",
		"נוב":        "november",
		"אוק":        "october",
		"שבת":        "saturday",
		"שנ׳":        "second",
		"ספט":        "september",
		"utc":        "utc",
		"שב׳":        "week",
		"שנה":        "year",
		"ב-":         "",
		"ה-":         "",
		"am":         "am",
		"pm":         "pm",
		"'":          "",
		",":          "",
		";":          "",
		"@":          "",
		"[":          "",
		"]":          "",
		"|":          "",
		" ":          " ",
		"+":          "+",
		"-":          "-",
		".":          ".",
		"/":          "/",
		":":          ":",
		"z":          "z",
	},
	TranslationRegexes: []ReplacementData{
		{regexp.MustCompile(`(?i)לפני (\d+) חודשים`), "$1 month ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) שבועות`), "$1 week ago"},
		{regexp.MustCompile(`(?i)בעוד (\d+) חודשים`), "in $1 month"},
		{regexp.MustCompile(`(?i)בעוד (\d+) שבועות`), "in $1 week"},
		{regexp.MustCompile(`(?i)לפני (\d+) שניות`), "$1 second ago"},
		{regexp.MustCompile(`(?i)בעוד (\d+) שניות`), "in $1 second"},
		{regexp.MustCompile(`(?i)לפני (\d+) ימים`), "$1 day ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) שעות`), "$1 hour ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) דקות`), "$1 minute ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) שנים`), "$1 year ago"},
		{regexp.MustCompile(`(?i)בעוד (\d+) ימים`), "in $1 day"},
		{regexp.MustCompile(`(?i)בעוד (\d+) שעות`), "in $1 hour"},
		{regexp.MustCompile(`(?i)בעוד (\d+) דקות`), "in $1 minute"},
		{regexp.MustCompile(`(?i)בעוד (\d+) שנים`), "in $1 year"},
		{regexp.MustCompile(`(?i)לפני (\d+) ימ׳`), "$1 day ago"},
		{regexp.MustCompile(`(?i)לפני יום (\d+)`), "$1 day ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) שע׳`), "$1 hour ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) דק׳`), "$1 minute ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) חו׳`), "$1 month ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) שנ׳`), "$1 second ago"},
		{regexp.MustCompile(`(?i)לפני (\d+) שב׳`), "$1 week ago"},
		{regexp.MustCompile(`(?i)בעוד (\d+) ימ׳`), "in $1 day"},
		{regexp.MustCompile(`(?i)בעוד יום (\d+)`), "in $1 day"},
		{regexp.MustCompile(`(?i)בעוד (\d+) שע׳`), "in $1 hour"},
		{regexp.MustCompile(`(?i)בעוד (\d+) דק׳`), "in $1 minute"},
		{regexp.MustCompile(`(?i)בעוד (\d+) חו׳`), "in $1 month"},
		{regexp.MustCompile(`(?i)בעוד (\d+) שנ׳`), "in $1 second"},
		{regexp.MustCompile(`(?i)בעוד (\d+) שב׳`), "in $1 week"},
	},
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(בעוד \d+ חודשים|בעוד \d+ שבועות|לפני \d+ חודשים|לפני \d+ שבועות|בעוד \d+ שניות|לפני \d+ שניות|בעוד \d+ דקות|בעוד \d+ ימים|בעוד \d+ שנים|בעוד \d+ שעות|לפני \d+ דקות|לפני \d+ ימים|לפני \d+ שנים|לפני \d+ שעות|בעוד \d+ דק׳|בעוד \d+ חו׳|בעוד \d+ ימ׳|בעוד \d+ שב׳|בעוד \d+ שנ׳|בעוד \d+ שע׳|בעוד יום \d+|לפני \d+ דק׳|לפני \d+ חו׳|לפני \d+ ימ׳|לפני \d+ שב׳|לפני \d+ שנ׳|לפני \d+ שע׳|לפני יום \d+)(\z|\W|_)`),
	RxExactCombined: regexp.MustCompile(`(?i)^(בעוד \d+ חודשים|בעוד \d+ שבועות|לפני \d+ חודשים|לפני \d+ שבועות|בעוד \d+ שניות|לפני \d+ שניות|בעוד \d+ דקות|בעוד \d+ ימים|בעוד \d+ שנים|בעוד \d+ שעות|לפני \d+ דקות|לפני \d+ ימים|לפני \d+ שנים|לפני \d+ שעות|בעוד \d+ דק׳|בעוד \d+ חו׳|בעוד \d+ ימ׳|בעוד \d+ שב׳|בעוד \d+ שנ׳|בעוד \d+ שע׳|בעוד יום \d+|לפני \d+ דק׳|לפני \d+ חו׳|לפני \d+ ימ׳|לפני \d+ שב׳|לפני \d+ שנ׳|לפני \d+ שע׳|לפני יום \d+)$`),
	RxKnownWords:    regexp.MustCompile(`(?i)^(.*?(?:\A|\W|_|\d))(החודש שעבר|השבוע שעבר|השנה שעברה|החודש הבא|השבוע הבא|השנה הבאה|יום חמישי|יום ראשון|יום רביעי|יום שלישי|באוקטובר|יום שישי|לאוקטובר|אוקטובר|באוגוסט|בדקה זו|בנובמבר|בסביבות|בספטמבר|בפברואר|בשעה זו|יום שבת|יום שני|לאוגוסט|לנובמבר|לספטמבר|לפברואר|אוגוסט|באפריל|בדצמבר|בינואר|בקירוב|חודשים|יום א׳|יום ב׳|יום ג׳|יום ד׳|יום ה׳|יום ו׳|לאפריל|לדצמבר|לינואר|לפנה״צ|נובמבר|ספטמבר|פברואר|שבועות|אחה״צ|אפריל|אתמול|בחודש|ביולי|ביוני|במארס|דצמבר|החודש|השבוע|חמישי|יום א|יום ב|יום ג|יום ד|יום ה|יום ו|ינואר|ליולי|ליוני|למארס|עכשיו|ראשון|רביעי|שלישי|שניות|שנייה|אוג׳|אוק׳|אפר׳|ביום|במאי|במרס|במרץ|בעוד|בערך|בשנה|בשעה|דצמ׳|דקות|היום|השנה|חודש|יולי|יוני|ימים|ינו׳|למאי|למרס|למרץ|לפני|מארס|נוב׳|ספט׳|פבר׳|שבוע|שישי|שניה|שנים|שעות|gmt|utc|אוג|אוק|אפר|דצמ|דקה|דק׳|חו׳|יול|יום|יונ|ינו|מאי|מחר|מרס|מרץ|נוב|ספט|עוד|פבר|שבת|שב׳|שנה|שני|שנ׳|שעה|שע׳|\+|\.|\[|\]|\||am|pm|ב-|ה-| |'|,|-|/|:|;|@|z)((?:\z|\W|_|\d).*)$`),
})
