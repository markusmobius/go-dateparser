...
var kl_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(for \d+ nalunaaquttap-akunnera siden|for \d+ sapaatip-akunnera siden|om \d+ nalunaaquttap-akunnera|for \d+ ulloq unnuarlu siden|om \d+ sapaatip-akunnera|for \d+ minutsi siden|for \d+ qaammat siden|for \d+ sekundi siden|om \d+ ulloq unnuarlu|for \d+ ukioq siden|om \d+ minutsi|om \d+ qaammat|om \d+ sekundi|om \d+ ukioq)(\z|\W|_)`),
