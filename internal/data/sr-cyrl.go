...
var sr_Cyrl_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(пре \d+ секунде|пре \d+ секунди|за \d+ секунди|за \d+ секунду|пре \d+ година|пре \d+ године|пре \d+ месеца|пре \d+ месеци|пре \d+ минута|пре \d+ недеља|пре \d+ недеље|за \d+ година|за \d+ годину|за \d+ месеци|за \d+ минута|за \d+ недеља|за \d+ недељу|за \d+ месец|за \d+ минут|пре \d+ дана|пре \d+ сата|пре \d+ сати|за \d+ дана|за \d+ сати|пре \d+ год|пре \d+ мес|пре \d+ мин|пре \d+ нед|пре \d+ сек|за \d+ год|за \d+ дан|за \d+ мес|за \d+ мин|за \d+ нед|за \d+ сат|за \d+ сек|пре \d+ г|пре \d+ д|пре \d+ м|пре \d+ н|пре \d+ с|пре \d+ ч|за \d+ г|за \d+ д|за \d+ м|за \d+ н|за \d+ с|за \d+ ч)(\z|\W|_)`),
...
var sr_Cyrl_BA_Locale = merge(&sr_Cyrl_Locale, LocaleData{
	...
	RxKnownWords: regexp.MustCompile(`(?i)^(.*?(?:\A|\W|_|\d))(прије подне|недјеља|сриједа|септ|gmt|utc|\+|\.|am|pm|ср|ут| |-|/|:|z)((?:\z|\W|_|\d).*)$`),
})
...
var sr_Cyrl_XK_Locale = merge(&sr_Cyrl_Locale, LocaleData{
	...
	RxKnownWords: regexp.MustCompile(`(?i)^(.*?(?:\A|\W|_|\d))(септ|gmt|utc|\+|\.|am|pm|ср|ут| |-|/|:|z)((?:\z|\W|_|\d).*)$`),
})
