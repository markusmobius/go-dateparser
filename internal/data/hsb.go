...
var hsb_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(pred \d+ tydzenjemi|pred \d+ hodzinami|pred \d+ sekundami|pred \d+ tydzenjom|pred \d+ mesacami|pred \d+ minutami|pred \d+ hodzinu|pred \d+ mesacom|pred \d+ sekundu|za \d+ tydzenjow|pred \d+ dnjemi|pred \d+ letami|pred \d+ minutu|za \d+ sekundow|pred \d+ dnjom|pred \d+ letom|za \d+ hodzinu|za \d+ mesacow|za \d+ minutow|za \d+ sekundu|pred \d+ hodz|pred \d+ tydz|za \d+ hodzin|za \d+ minutu|za \d+ tydzen|pred \d+ dnj|pred \d+ mes|pred \d+ min|pred \d+ sek|za \d+ dnjow|za \d+ mesac|za \d+ dzen|za \d+ hodz|za \d+ leto|za \d+ tydz|pred \d+ d|pred \d+ h|pred \d+ l|pred \d+ m|pred \d+ s|za \d+ dnj|za \d+ let|za \d+ mes|za \d+ min|za \d+ sek|za \d+ d|za \d+ h|za \d+ l|za \d+ m|za \d+ s)(\z|\W|_)`),
