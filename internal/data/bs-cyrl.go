...
var bs_Cyrl_Locale = merge(nil, LocaleData{
	...
	RxCombined:      regexp.MustCompile(`(?i)(\A|\W|_)(пре \d+ секунди|за \d+ секунди|пре \d+ година|пре \d+ годину|пре \d+ месеци|пре \d+ минута|пре \d+ недеља|пре \d+ недељу|пре \d+ секунд|за \d+ година|за \d+ годину|за \d+ месеци|за \d+ минута|за \d+ недеља|за \d+ недељу|за \d+ секунд|пре \d+ месец|пре \d+ минут|за \d+ месец|за \d+ минут|пре \d+ дана|пре \d+ сати|за \d+ дана|за \d+ сати|пре \d+ дан|пре \d+ сат|за \d+ дан|за \d+ сат)(\z|\W|_)`),
